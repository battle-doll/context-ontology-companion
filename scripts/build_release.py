#!/usr/bin/env python3
"""Build a deterministic local source-preview ZIP; never uploads or publishes."""
...
target=out/(ROOT.name+"-0.1.0-draft.1-source-preview.zip")
...
print(json.dumps({"artifact":target.name,"sha256":hashlib.sha256(target.read_bytes()).hexdigest(),"status":"source_preview_not_published"}))
