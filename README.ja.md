...
`demo` は合成例、一時保存、模擬承認を使用します。別のローカル試作版には、永続ローカル保存、認証付きループバックの人による確認画面、手動設定する stdio を実装しました。ローカルの合成テストは成功しています。macOS の Codex 0.153.3 で、試験用と明記したサンプルを用いた手動設定のローカル stdio スモークテストも成功しました。ホスト型 ChatGPT 連携、プラグインマーケットプレイスでのインストール・Directory 公開、実際の人による承認は未検証です。本番での実データ利用と本番認証は未対応です。
...
リポジトリのプレビューが公開されたら、プラグイン対応の Codex CLI で完全なパッケージをインストールします。
...
[Windows クイックスタート](docs/WINDOWS_QUICKSTART.md) · [現在のリリース状況](release-state.json)。GitHub マーケットプレイスからのインストールと universal Directory への公開は別の状態です。以下の試験証拠は記録時点のローカル検証結果です。その後の公開・OS 検証状況はリリース状態ファイルを参照してください。
...
対応対象 OS は **macOS・Windows・Linux** です。ソースの移植性、実行済み OS テスト、プラグインホストの動作は別々に確認します。このドラフトは全 OS のテスト成功を主張しません。
...
| プラグインホスト連携 | macOS Codex 0.153.3 の手動設定ローカル stdio スモークテスト成功。ホスト型 ChatGPT・マーケットプレイス/Directory・実際の人による承認は未検証 |
